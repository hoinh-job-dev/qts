<link rel="stylesheet" type="text/css" href="<?php echo base_url('css/operator/view.css'); ?>">
		...
		<div class="l-main js-height">
			<div class="l-page-">
                ...
                <!-- Begin form search -->
                <form action="<?php echo base_url('Operator/editTokenList'); ?>" method="post" id="search" class="search">
                ...
                <!-- View result search -->
                <form action="<?php echo base_url('Operator/editTokenList'); ?>" method="post" id="update">
                <div class="whitebox">
                    <h3>還元の関係情報一覧</h3>                    
                    <?php if (isset($status)) { if('success' == $status){ ?>
                        <div class ="whitebox" style="color: blue;">
                            <?php echo "還元コードの変更を正常に行いました。"; ?>
                        </div>
                    <?php }else{ ?>
                    ...
                    <ul id="button">
                        <?php if ($this->CI->check_export_csv_permission()){ ?>
                            <input type="button" id="update" value="CSV出力" onclick="exportCsvFromSearch('exportCsvEditTokenList');"/>
                        <?php } ?>
